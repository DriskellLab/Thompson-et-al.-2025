...
library(CellChat)#install BiocNeighbors using BiocManager
library(patchwork)
options(stringsAsFactors = FALSE)

```

#1. Subset to Basal Krtno & Dermal Pocket Only
```{r}
##Subset
Mon6_cc <- subset(Mon6, idents = c('0 Pericyte', '1 Basal Krtno', '3 Fibroblast', '6 BV', '8 Div Krtno',
                     '16 LV'), invert = FALSE)#subset
Mon6_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('Peri', '1', '3', '6', '8', '16')
...
names(new.cluster.ids) <- levels(Mon6_cc)
Mon6_cc <- RenameIdents(Mon6_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(Mon6_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(Mon6_cc)
...
```{r}
save(cellchat, file = '/media/sean/T2TB/Object/6mo_Pig_cellchat_v2_refined.Rdata')
```
...
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
umap_cols <- c('#7fcdbb', '#002783', '#64a7e9', '#238b45', '#4d8ad5',
                     '#96ccff', '#8c6bb1', '#0c3695', '#2565b5', '#BB0000',
                     '#790001', '#16539e', '#00206a', '#dc0000', '#900101',
                     '#ffab4b', '#88419d', '#9decff')#string vector of my custom colors for UMAP
cols_i <- as.numeric(levels(Mon6_cc))
cols_i[1] <- 0#since cellchat can't take 0 as a cluster id, now need to replace 'chr_id' --> NA from as.numeric w/ 0 for initial cluster
...
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
par(mfrow=c(1,1))
n_loop <- 1#storage variable to iterate through
while (n_loop <= length(pathways.show[1:56])) {
...
while (n_loop <= length(pathways.show[58:length(pathways.show)])) {
  #print(pathways.show[n_loop])#test
  print(netVisual_heatmap(cellchat, signaling = pathways.show[n_loop], color.heatmap = "Reds"))#want to view one pathway at a time
  n_loop <- n_loop + 1#iterate to next pathway
}
...
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
load("/media/sean/T2TB/Object/6mo_Pig_cellchat_v2_refined.Rdata")
cellchat_6mo <- cellchat
```

##Set Cluster Order for Heatmaps
```{r}
cellchat_6mo <- updateClusterLabels(cellchat_6mo, new.order = c('1', '8', '3', 'Peri', '6', '16'))
umap_cols <- c('#7fcdbb', '#002783', '#64a7e9', '#238b45', '#4d8ad5',
                     '#96ccff', '#8c6bb1', '#0c3695', '#2565b5', '#BB0000',
                     '#790001', '#16539e', '#00206a', '#dc0000', '#900101',
                     '#ffab4b', '#88419d', '#9decff')#string vector of my custom colors for UMAP
umap_cols <- c('#002783', '#2565b5',  '#238b45', '#7fcdbb','#8c6bb1', '#88419d')#reorder to match the reordered clusters
names(umap_cols) <- umap_cols#Cellchat V2.0+ requires named color vector for manual coloring, not needed in my original analyses in V1
pathways.show <- cellchat_6mo@netP$pathways#store pathways with significant communication
#pathways.show#print to notebook the pathways
```

save the ordered cellchat object
```{r}
save(cellchat_6mo, file = '/media/sean/T2TB/Object/6mo_Pig_cellchat_dermalpocket_ordered.Rdata')
```

#re-initialize common inputs if necessary
```{r}
groupSize <- as.numeric(table(cellchat_6mo@idents)) # number of cells in each cell group
pathways.show <- cellchat_6mo@netP$pathways#store pathways with significant communication
mat <- cellchat_6mo@net$weight
umap_cols <- c('#002783', '#2565b5',  '#238b45', '#7fcdbb','#8c6bb1', '#88419d')#reorder to match the reordered clusters
names(umap_cols) <- umap_cols#Cellchat V2.0+ requires named color vector for manual coloring, not needed in my original analyses in V1
...
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Ordered/Cluster_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
# Hierarchy plot
vertex.receiver = seq(1,4) # a numeric vector.
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Ordered/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object

n_loop <- 1#storage variable to iterate through
while (n_loop <= length(pathways.show)) {
  netVisual_aggregate(cellchat_6mo, signaling = pathways.show[n_loop],  vertex.receiver = vertex.receiver,
...
Vignette does not do this automated so I wrote a nested loop to print circle plots for each ligand receptor pair within each pathway
```{r fig.height=7, fig.width=7}
path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Ordered/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object

n_pathway <- 1#storage variable to iterate through pathways
while (n_pathway <= length(pathways.show)) {
  LR.temp <- extractEnrichedLR(cellchat_6mo, signaling = pathways.show[n_pathway], 
                               geneLR.return = FALSE, thresh = 0.05,                      
                               enriched.only = TRUE)#return sig. LR pairs from a single pathway
  ...
  while (n_lr <= nrow(LR.temp)) {
    print(paste(pathways.show[n_pathway], LR.temp[n_lr, ], sep = ': '))
    vertex.receiver = seq(1, (nrow(LR.temp) + 1)) # a numeric vector
    p1 <- netVisual_individual(cellchat_6mo, signaling = pathways.show[n_pathway],  
                         pairLR.use = LR.temp[n_lr, ], vertex.receiver = vertex.receiver,
                         arrow.size = 1, arrow.width = 1.5, color.use = umap_cols)
    ...
    png(filename = paste0(pathways.show[n_pathway], '-', LR.temp[n_lr, ], '.png', sep = ''),
        width = 7, height = 7, units = 'in', res = 300)#start graphics device
    netVisual_individual(cellchat_6mo, signaling = pathways.show[n_pathway],  
                         pairLR.use = LR.temp[n_lr, ], vertex.receiver = vertex.receiver,
                         arrow.size = 1, arrow.width = 1.5, color.use = umap_cols)
    dev.off()
...
```{r}
ptm = Sys.time()
# Compute the network centrality scores##Visualize dominant senders and receivers (Heatmap)
```{r fig.height=9, fig.width=12}
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_6mo, pattern = "outgoing", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_6mo, pattern = "incoming", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht1 + ht2
p1 <- ht1 + ht2

path_out <- '~/Sean/Plots/6mo_EpiDer_Refined/Ordered/Net_Heatmap/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('6mo_DermalPocket_AllPathways_SenderReceiver_Heatmap', '.png', sep = ''),
...
POI <- c("BMP", "NRG", "PECAM1", "NOTCH", "TGFb", "EPHA", "WNT", "EPHB", "ANGPT", "ANGPTL", "VEGF", "CDH5", "PDGF", "FGF", "ncWNT", "EGF", "IL6", "CCL", "KIT", "EDN", "CXCL", "GHRELIN", "RA")
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_6mo, signaling = POI, pattern = "outgoing", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_6mo, signaling = POI, pattern = "incoming", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht1 + ht2
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('Pig6mo_DermalPocket_SenderReceiver_Heatmap', '.png', sep = ''),
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('Pig6mo_DermalPocket_Figure_SenderReceiver_Heatmap', '.png', sep = ''),
        width = 12, height = 7, units = 'in', res = 300)#start graphics device
ht1 + ht2
dev.off()
...
cellchat_6mo <- netAnalysis_computeCentrality(cellchat_6mo, slot.name = "netP") # the slot 'netP' means the inferred intercellular communication network of signaling pathways
# Visualize the computed centrality scores using heatmap, allowing ready identification of major signaling roles of cell groups
netAnalysis_signalingRole_network(cellchat_6mo, signaling = pathways.show, width = 8, height = 2.5, font.size = 10)
```

##Visualize dominant senders and receivers (Scatter Plot)
```{r}
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
gg1 <- netAnalysis_signalingRole_scatter(cellchat_6mo, color.use = umap_cols)
#> Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
# Signaling role analysis on the cell-cell communication networks of interest
gg2 <- netAnalysis_signalingRole_scatter(cellchat_6mo, signaling = c("PDGF", "ANGPTL"), color.use = umap_cols)
