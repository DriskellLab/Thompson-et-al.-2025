...
#1. Subset to Basal Krtno & Upper Dermis (Epidermal-dermal interactions prior to dermal pocket formation)
Using updated CellChat V2
```{r}
##Subset

E90_cc <- subset(e90, idents = c('0 Basal Krtno', '4 PF', '7 Div Krtno', '9 BV', '12 Pericyte', '23 LV'), invert = FALSE)#subset
E90_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('BK', '4', '7', '9', '12', '23')
...
names(new.cluster.ids) <- levels(E90_cc)
E90_cc <- RenameIdents(E90_cc, new.cluster.ids)
```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(E90_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(E90_cc)
...
```{r}
save(cellchat, file = '/media/sean/T2TB/Object/E90_Pig_cellchat_v2_refined_noBB.Rdata')
```
...
load("/media/sean/T2TB/Object/E90_Pig_cellchat_v2_refined.Rdata")
cellchat_e90_noBB <- cellchat
```

##Set Cluster Order for Heatmaps
```{r}
#cellchat_e90_noBB@idents <- factor(cellchat_e90_noBB@idents, levels = c('BK', '7', '4', '12', '9', '23')) # show factor levels of the cell labels
cellchat_e90_noBB <- updateClusterLabels(cellchat_e90_noBB, new.order = c('BK', '7', '4', '12', '9', '23'))
umap_cols <- c('#002783', '#64a7e9', '#41ab5d', '#16539e', '#00441b', 
                     '#0c6395', '#006837', '#2565b5', '#a1d99b', '#8c6bb1', 
                     '#238b45', '#1179a3', '#7fcdbb', '#790001', '#96ccff', 
                     '#f768a1', '#00206a', '#bdbdbd', '#ae017e', '#ffab4b', 
                     '#cc0000', '#BB0000', '#164c9a', '#88419d', '#fc8d59')#string vector of my custom colors for UMAP
umap_cols <- c('#002783', '#2565b5', '#00441b','#7fcdbb', '#8c6bb1', '#88419d')#reorder to match the reordered clusters
...
```{r}
save(cellchat_e90_noBB, file = '/media/sean/T2TB/Object/e90_cellchat_epi_der_ordered_noBB.Rdata')
```

```{r}
load('/media/sean/T2TB/Object/e90_cellchat_epi_der_ordered_noBB.Rdata')
```

#re-initialize common inputs if necessary
```{r}
groupSize <- as.numeric(table(cellchat_e90_noBB@idents)) # number of cells in each cell group
pathways.show <- cellchat_e90_noBB@netP$pathways#store pathways with significant communication
mat <- cellchat_e90_noBB@net$weight

#umap_cols <- c('#002783', '#64a7e9', '#41ab5d', '#16539e', '#00441b', 
#                     '#0c6395', '#006837', '#2565b5', '#a1d99b', '#8c6bb1', 
#                     '#238b45', '#1179a3', '#7fcdbb', '#790001', '#96ccff', 
#                     '#f768a1', '#00206a', '#bdbdbd', '#ae017e', '#ffab4b', 
#                     '#cc0000', '#BB0000', '#164c9a', '#88419d', '#fc8d59')
#
#cols_i <- as.numeric(levels(E90_cc))
...
path_out <- '~/Sean/Plots/E90_EpiDer_Refined/Ordered_noBB/Cluster_CirclePlot/'
...
path_out <- '~/Sean/Plots/E90_EpiDer_Refined/Ordered_noBB/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object

n_loop <- 1#storage variable to iterate through
while (n_loop <= length(pathways.show)) {
  netVisual_aggregate(cellchat_e90_noBB, signaling = pathways.show[n_loop],  vertex.receiver = vertex.receiver,
...
```{r fig.height=7, fig.width=7}
path_out <- '~/Sean/Plots/E90_EpiDer_Refined/Ordered_noBB/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object

n_pathway <- 1#storage variable to iterate through pathways
while (n_pathway <= length(pathways.show)) {
  LR.temp <- extractEnrichedLR(cellchat_e90_noBB, signaling = pathways.show[n_pathway], 
  ...
  while (n_lr <= nrow(LR.temp)) {
    ...
    p1 <- netVisual_individual(cellchat_e90_noBB, signaling = pathways.show[n_pathway],  
    ...
    png(filename = paste0(pathways.show[n_pathway], '-', LR.temp[n_lr, ], '.png', sep = ''),
        width = 7, height = 7, units = 'in', res = 300)#start graphics device
    netVisual_individual(cellchat_e90_noBB, signaling = pathways.show[n_pathway],  
                         pairLR.use = LR.temp[n_lr, ], vertex.receiver = vertex.receiver,
                         arrow.size = 1, arrow.width = 1.5, color.use = umap_cols)
    dev.off()
...
cellchat_e90_noBB <- netAnalysis_computeCentrality(cellchat_e90_noBB, slot.name = "netP") # the slot 'netP' means the inferred intercellular communication network of signaling pathways
# Visualize the computed centrality scores using heatmap, allowing ready identification of major signaling roles of cell groups
netAnalysis_signalingRole_network(cellchat_e90_noBB, signaling = pathways.show, width = 8, height = 2.5, font.size = 10, color.use = umap_cols)
```

##Visualize dominant senders and receivers (Scatter Plot)
```{r}
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
gg1 <- netAnalysis_signalingRole_scatter(cellchat_e90_noBB, color.use = umap_cols)
#> Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
# Signaling role analysis on the cell-cell communication networks of interest
gg2 <- netAnalysis_signalingRole_scatter(cellchat_e90_noBB, signaling = c("PDGF", "ANGPTL"), color.use = umap_cols)
...
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_e90_noBB, pattern = "outgoing", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_e90_noBB, pattern = "incoming", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht1 + ht2
p1 <- ht1 + ht2
...
path_out <- '~/Sean/Plots/E90_EpiDer_Refined/Ordered_noBB/Net_Heatmap/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_e90_noBB, signaling = POI, pattern = "outgoing", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_e90_noBB, signaling = POI, pattern = "incoming", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht1 + ht2
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('PigE90_notDermalPocket_noBB_SenderReceiver_Heatmap', '.png', sep = ''),
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('PigE90_notDermalPocket_noBB_Figure_SenderReceiver_Heatmap', '.png', sep = ''),
        width = 12, height = 7, units = 'in', res = 300)#start graphics device
ht1 + ht2
dev.off()
