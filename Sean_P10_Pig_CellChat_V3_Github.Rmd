...
#1b. Subset the Seurat object (Dermal Pocket, just core basal state)
```{r}
##Subset
D10_cc <- subset(D10, idents = c('3 Basal Krtno',  '8 Div Krtno', '9 Pericyte',
                     '10 BV', '11 PF', '17 LV'), invert = FALSE)#subset the fibroblast supercluster
D10_cc#view the subset

##Rename clusters to their #'s since this cleans up the CellChat plots
cluster_names <- c('3', '8', '9', '10', '11', '17')
...
names(new.cluster.ids) <- levels(D10_cc)
D10_cc <- RenameIdents(D10_cc, new.cluster.ids)

```

#Initialize cellchat object and db
```{r}
data.input <- GetAssayData(D10_cc, assay = "SCT", slot = "data") # normalized data matrix
labels <- Idents(D10_cc)
...
```{r}
save(cellchat, file = '/media/sean/T2TB/Object/p10_cellchat_dermalpocket_refined.Rdata')
```
...
#Manually order the CellChat object for circle plotting
```{r}
cellchat <- updateClusterLabels(cellchat, new.order = c('3', '8', '11', '9', '10', '17'))
umap_cols <- c('#4d8ad5', '#64a7e9', '#4d91d5', '#002783', '#16539e',
                     '#238b45', '#00206a', '#0c3695', '#2565b5', '#7fcdbb',
                     '#8c6bb1', '#00441b', '#900101', '#3b7bca', '#41ab5d', 
                     '#006837', '#ffab4b', '#88419d')#string vector of my custom colors for UMAP
...
#____
path_out <- '~/Sean/Plots/P10_EpiDer_Refined/Ordered/Cluster_CirclePlot/'
...
path_out <- '~/Sean/Plots/P10_EpiDer_Refined/Ordered/Path_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
path_out <- '~/Sean/Plots/P10_EpiDer_Refined/Ordered/LR_Pair_CirclePlot/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
...
load("/media/sean/T2TB/Object/p10_cellchat_dermalpocket_refined.Rdata")
cellchat_p10 <- cellchat
```

##Set Cluster Order for Heatmaps
```{r}
cellchat_p10 <- updateClusterLabels(cellchat_p10, new.order = c('3', '8', '11', '9', '10', '17'))
umap_cols <- c('#4d8ad5', '#64a7e9', '#4d91d5', '#002783', '#16539e',
                     '#238b45', '#00206a', '#0c3695', '#2565b5', '#7fcdbb',
                     '#8c6bb1', '#00441b', '#900101', '#3b7bca', '#41ab5d', 
                     '#006837', '#ffab4b', '#88419d')#string vector of my custom colors for UMAP
...
```{r}
save(cellchat_p10, file = '/media/sean/T2TB/Object/p10_cellchat_dermalpocket_refined_ordered.Rdata')
```

#re-initialize common inputs if necessary
```{r}
groupSize <- as.numeric(table(cellchat_p10@idents)) # number of cells in each cell group
pathways.show <- cellchat_p10@netP$pathways#store pathways with significant communication
mat <- cellchat_p10@net$weight
umap_cols <- c('#002783',  '#2565b5', '#00441b','#7fcdbb', '#8c6bb1', '#88419d')#reorder to match the reordered clusters
names(umap_cols) <- umap_cols
```
...
cellchat_p10 <- netAnalysis_computeCentrality(cellchat_p10, slot.name = "netP") # the slot 'netP' means the inferred intercellular communication network of signaling pathways
# Visualize the computed centrality scores using heatmap, allowing ready identification of major signaling roles of cell groups
netAnalysis_signalingRole_network(cellchat_p10, signaling = pathways.show, width = 8, height = 2.5, font.size = 10, color.use = umap_cols)
```

##Visualize dominant senders and receivers (Scatter Plot)
```{r}
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
gg1 <- netAnalysis_signalingRole_scatter(cellchat_p10, color.use = umap_cols)
#> Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
# Signaling role analysis on the cell-cell communication networks of interest
gg2 <- netAnalysis_signalingRole_scatter(cellchat_p10, signaling = c("PDGF", "ANGPTL"), color.use = umap_cols)
...
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_p10, pattern = "outgoing", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_p10, pattern = "incoming", color.use = umap_cols, width = 12, height = 15, font.size = 5)
ht1 + ht2
p1 <- ht1 + ht2
path_out <- '~/Sean/Plots/P10_EpiDer_Refined/Ordered/Net_Heatmap/'
setwd(path_out)#need to change working directory to export plots b/c the plot is not a ggplot object
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('PigP10_DermalPocket_AllPathways_SenderReceiver_Heatmap', '.png', sep = ''),
...
ht1 <- netAnalysis_signalingRole_heatmap(cellchat_p10, signaling = POI, pattern = "outgoing", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat_p10, signaling = POI, pattern = "incoming", color.use = umap_cols,
                                         width = 12, height = 7, font.size = 7)
ht1 + ht2
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('PigP10_DermalPocket_Refined_SenderReceiver_Heatmap', '.png', sep = ''),
...
#use png graphics device to export the cellchat plots (gives control over dimensions and naming versus netVisual function)
png(filename = paste0('PigP10_DermalPocket_Figure_SenderReceiver_Heatmap', '.png', sep = ''),
        width = 12, height = 7, units = 'in', res = 300)#start graphics device
ht1 + ht2
dev.off()
